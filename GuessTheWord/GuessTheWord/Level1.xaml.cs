...
namespace GuessTheWord
{
    ...
    public sealed partial class Level1 : Page
    {
        ...
        {
            if (((txtB1.Text + txtB2.Text + txtB3.Text + txtB4.Text + txtB5.Text)).ToLower().Equals("smile"))
            {
                ...
                try
                {
                    StorageFile file = await storageFolder.GetFileAsync(fileInPC);
                    if (file != null)
                    {
                ...
                //StorageFile createFile = await storageFolder.CreateFileAsync(fileInPC);
                //await Windows.Storage.FileIO.WriteTextAsync(createFile, sLevel);
